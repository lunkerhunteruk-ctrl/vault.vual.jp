...
export function AuthModal({ open, onClose, onSuccess }: AuthModalProps) {
  ...
  const isInAppBrowser = typeof navigator !== "undefined" &&
    /Instagram|FBAN|FBAV|Line|Twitter|TikTok/i.test(navigator.userAgent);
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // null means redirect (mobile) — page will reload
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {isInAppBrowser ? (
          /* In-app browser: open in Safari/Chrome instead */
          <button
            onClick={() => {
            ...
            className="w-full py-4 flex items-center justify-center gap-3 border border-white/20 text-white/70 rounded-lg hover:border-white/40 transition-colors"
          >
            <span className="text-[12px] tracking-[2px] font-light">
              OPEN IN BROWSER TO SIGN IN
            </span>
          </button>
        ) : (
          /* Normal browser: Google Sign-In */
          <button
